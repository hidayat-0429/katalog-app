...
function getUsageContext(productName: string, categoryName: string): string | null {
  const name = productName.toLowerCase();
  const category = categoryName.toLowerCase();
  
  // Context mapping based on mushroom type
  if (name.includes('shiitake')) return 'Perfect for Ramen & Asian Cuisine';
  if (name.includes('champignon') || name.includes('button')) return 'Ideal untuk Pizza & Pasta';
  ...
  // Default by category
  if (category.includes('olahan')) return 'Siap Goreng & Sajikan';
...
export default function ProductCard({
  ...
  const usageContext = getUsageContext(name, categoryName);
