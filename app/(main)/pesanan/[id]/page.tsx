...
import { ChevronRight, MapPin, Package, AlertCircle, Calendar, Building2, ClipboardCheck } from "lucide-react";
...
export default async function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const stepLabels = ["Menunggu", "Diproses", "Dikirim", "Selesai"];
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8 print:hidden">
        <nav className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400 font-sans">
          <Link href="/pesanan" className="hover:text-neutral-900 dark:hover:text-neutral-100 transition-colors duration-150 ease-out">Pesanan Saya</Link>
      ...
      <div className="hidden print:flex items-center justify-between border-b border-stone-300 pb-4 mb-6 font-sans">
        ...
        <div className="text-right text-xs">
          ...
          <p className="text-stone-500">{new Date(order.createdAt).toLocaleDateString("id-ID", { dateStyle: "long" })}</p>
        </div>
      </div>
      ...
      {!isCanceled ? (
        ...
        <div className="mb-6 bg-semantic-danger-light dark:bg-semantic-danger-darkBg border border-semantic-danger-DEFAULT/30 rounded-xl p-4 flex items-start gap-3 print:hidden">
          ...
          <div>
            <p className="font-semibold text-sm text-semantic-danger-dark dark:text-red-300">Pesanan Dibatalkan</p>
            <p className="text-xs text-semantic-danger-dark/80 dark:text-red-300/80 mt-0.5">Pesanan ini telah dibatalkan dan stok produk telah dikembalikan ke gudang.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        ...
        <div className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl p-5 flex flex-col gap-4 font-sans">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 flex items-center gap-1.5">
            <Package className="w-3.5 h-3.5" /> Informasi Tujuan
          </h2>

          <div className="space-y-3 text-xs">
            <div>
              <p className="text-neutral-500 dark:text-neutral-400 mb-0.5 flex items-center gap-1"><Calendar className="w-3 h-3" /> Tanggal Transaksi</p>
              <p className="font-medium text-neutral-900 dark:text-neutral-100">
                {new Date(order.createdAt).toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
              </p>
            </div>
            <div>
              <p className="text-neutral-500 dark:text-neutral-400 mb-0.5 flex items-center gap-1"><Building2 className="w-3 h-3" /> Pemesan</p>
              <p className="font-semibold text-neutral-900 dark:text-neutral-100">{order.buyerName || order.user.name}</p>
              {(order.companyName || order.user.companyName) && (
                <p className="text-neutral-500 dark:text-neutral-400 mt-0.5">{order.companyName || order.user.companyName}</p>
              )}
            </div>
            <div>
              <p className="text-neutral-500 dark:text-neutral-400 mb-0.5 flex items-center gap-1"><MapPin className="w-3 h-3" /> Alamat Kirim</p>
              <p className="text-neutral-900 dark:text-neutral-100 leading-relaxed">{order.shippingAddress}</p>
            </div>
            {order.notes && (
              <div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
                <p className="text-neutral-500 dark:text-neutral-400 mb-0.5">Catatan</p>
        ...
        <div className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl overflow-hidden md:col-span-2 font-sans">
          <div className="px-5 py-3.5 border-b border-neutral-200 dark:border-neutral-700">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Rincian Barang</h2>
          </div>
            ...
            {order.items.map((item) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-3 px-5 py-4 items-center text-sm">
                <div className="md:col-span-6">
                  <p className="font-semibold text-neutral-900 dark:text-neutral-100 text-sm">{item.productName || item.product?.name}</p>
          ...
          <div className="px-5 py-4 border-t border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50 flex justify-between items-center">
            <span className="font-semibold text-sm text-neutral-900 dark:text-neutral-100">Total Pembayaran</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-5 border-t border-neutral-200 dark:border-neutral-700 print:hidden">
        {!isCanceled ? (
          <WhatsAppOrderButton
            ...
            items={order.items.map((i) => ({ productName: i.productName || i.product?.name || "Produk", quantity: i.quantity }))}
          />
        ) : <div />}
